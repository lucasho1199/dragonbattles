...
contract DragonBattlesToken is Ownable, ERC20 {
    using SafeMath for uint256;
    uint256 public maxSupply = 500 * 10**6 * 10**18;
    address public dragonFarm;
    address public devAddress;
    uint256 public taxFee = 0;
    mapping(address => bool) private _isExcludedFromFee;

    constructor() ERC20("DragonBattlesToken", "DRB") {
        _mint(_msgSender(), maxSupply);
        _isExcludedFromFee[_msgSender()] = true;
        dragonFarm = _msgSender();
    ...
    function _transfer(
        ...
        if (
            ...
            (_isExcludedFromFee[recipient] && recipient != dragonFarm)
        ) {
            takeFee = false;
        }

        if (takeFee == true && taxFee > 0) {
            uint256 _taxFee = amount.mul(taxFee).div(10000);
    ...
    function setFarm(address _farm) external onlyOwner {
        if (dragonFarm == address(0)) {
            require(msg.sender == owner(), "not owner");
        } else {
            require(msg.sender == dragonFarm, "not farm");
        }
        dragonFarm = _farm;
    }
